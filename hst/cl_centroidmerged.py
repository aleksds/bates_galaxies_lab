...
with PdfPages('ad_centroid.pdf') as pdf:
    for j in range(0,len(Galaxies)):
        fig = plt.figure()
        plt.scatter(Galaxies[j][1],Galaxies[j][2], label='current')
        for i in range(0,len(collection)):
            ...
            coor = plot_image(Galaxies[j][1],Galaxies[j][2])
            plt.scatter(coor[0],coor[1],label = yolo[i])
        plt.xlabel('x')
        plt.ylabel('y')
        plt.xlim(Galaxies[j][1]-10,Galaxies[j][1]+10)
        plt.ylim(Galaxies[j][2]-10,Galaxies[j][2]+10)
