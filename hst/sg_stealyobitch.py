...
def plot_image(posx,posy, count, prevstds):
    ...
    # RECURSION BITCH
    if count < 100 and (xstd > 0.09 or ystd > 0.1) and (xstd <= prevstds[0] and ystd <= prevstds[1]):
...
for j in range(0,len(Galaxies)):
    ...
    #ystds[j] = 1
        ...
        for i in range(0,len(filters)):
            ...
            mxs[i], mys[i], mstdx[i], mstdy[i], count = plot_image(Galaxies[j][1],Galaxies[j][2], 0, [1000, 1000])
