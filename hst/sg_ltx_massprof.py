    ...
    for root, dirs, filenames in os.walk(datadir):
        for f in filenames:
            if "UVIS" in f and ".xls" in f:
                ...
                plt.ylabel('Stellar Mass ($ M_{\odot}$)',fontsize=16)
                ...
                plt.title('Mass Density vs Radius for '+ gal.name, fontsize = 20)
                plt.xlabel('Radius (kpc)', fontsize = 18)
                #plt.ylabel('Mass Density (solar masses / kpc^2)')
                plt.ylabel('Stellar Mass Surface Density ($ M_{\odot} / kpc^{2}$)',fontsize=18)
