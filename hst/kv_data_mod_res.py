...
#This is a version specifically for King's Goals/Tasks 
...
dx = dy = 60
with PdfPages('kv_J0826_Visual.pdf') as pdf:
    for g in range(0,len(galaxies)):
        ...
        for f in range(0,len(filters)):
            for t in range(0,len(type)):
                file = glob.glob('/Volumes/physics/linux-lab/data/galfit/kingfiles/kval_'+galaxies[g]+filters[f]+'_fine_sersic.fits') 
                                 
                multi = fits.open(file[0])
                
                data, data_header = multi[1].data, multi[1].header
                model, model_header = multi[2].data, multi[2].header
        ...
        #code to read in galfitm output from simultaneous fit
        file = glob.glob('/Volumes/physics/linux-lab/data/galfit/kingfiles/kval_'+galaxies[g]+'_F814W_F475W_output.fits')
        
        multi = fits.open(file[0])
        #do F475W first 
        data, data_header = multi[1].data, multi[1].header
        model, model_header = multi[3].data, multi[3].header
        res, res_header = multi[5].data, multi[5].header
...
os.system('open %s &' % 'kv_J0826_Visual.pdf')
