...
if togetherness == 'independent':
    ...
    for w in range(0,12):
        file = time+'_'+model+'_'+plate+'_'+togetherness+'_'+imgsize+'_psf'+psf+'/'+galaxies[w]+'_'+filters[1]+'_'+filters[0]+'_'+model+'_output.galfit.01.band'
        with open(file) as f:
            content = f.readlines()
...
text = open(file,'w')
text.write('ID                f_475        ivar_475 f_814        ivar_814 z\n')
for w in range(0,12):
    text.write(galaxies[w]+'    '+str(round(fluxes[w][0],4))+'    '+str(round(ivar[w][0],4))+'    '+str(round(fluxes[w][1],4))+'    '+str(round(ivar[w][1],4))+'    '+str(redshifts[w])+'\n')
text.close()
