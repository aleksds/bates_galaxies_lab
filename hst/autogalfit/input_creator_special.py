...
import datetime
import sys
#sys.argv[1] is psf or sersic (model), sys.arg[2] is coarse or fine (plate scale), sys.arg[3] is a number (image region size), sys.arg[4] is coarse or fine (psf)
...
for w in range(0,12):
    for i in range(0,2):
        if not os.path.exists(time+'_'+model+'_'+plate+'_'+kind[0]+'_'+model+'_'+plate+'_'+imgsize+'_psf'+psf):
            os.makedirs(time+'_'+model+'_'+plate+'_'+kind[0]+'_'+model+'_'+plate+'_'+imgsize+'_psf'+psf)


        file = time+'_'+model+'_'+plate+'_'+kind[0]+'_'+model+'_'+plate+'_'+imgsize+'_psf'+psf+'/'+galaxies[w]+'_'+filters[i]+'_'+model+'_input.txt'
        text = open(file,'w')
        text.write('#  Input menu file: '+galaxies[w]+'_'+filters[i]+'_'+model+'_'+plate+'\n') #probably not essential
...
file = time+'_'+model+'_'+plate+'_'+kind[0]+'_'+model+'_'+plate+'_'+imgsize+'_psf'+psf+'/'+'psf_independent_run_files.sh'
text = open(file,'w')
text.write('shopt -s expand_aliases\n')
text.write('source ~/.bash_profile\n')
for w in range(0,12):
    for i in range(0,2):
        text.write('galfitm '+galaxies[w]+'_'+filters[i]+'_'+model[0]+'_'+plate+'_input.txt\n')
text.close()

file = time+'_'+model+'_'+plate+'_'+kind[0]+'_'+model+'_'+plate+'_'+imgsize+'_psf'+psf+'/'+'sersic_independent_run_files.sh'
text = open(file,'w')
text.write('shopt -s expand_aliases\n')
text.write('source ~/.bash_profile\n')
for w in range(0,12):
    for i in range(0,2):
        text.write('galfitm '+galaxies[w]+'_'+filters[i]+'_'+model[1]+'_'+plate+'_input.txt\n')
text.close()
