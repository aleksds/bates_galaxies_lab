...
# run bgl_input_magmag_coarse.py sersic coarse 100 coarse simultaneous s_index
    ...
    for w in range(0,ngal):
        ...
        for j in range(0, nmag):
            ...
            for q in range(0, nmag):
                ...
                file = time+'_'+model+'_'+plate+'_'+togetherness+'_'+imgsize+'_psf'+psf+'/'+galaxies[w]+'_F814WandF475W_'+model+'_m1'+str(q)+'_m2'+str(j)+'_input.txt'
                text = open(file,'w')
    
                text.write('#  Input menu file: '+galaxies[w]+'_F814WandF475W_'+model+'_m1'+str(q)+'_m2'+str(j)+'_input.txt\n')
                ...
                text.write('B) '+galaxies[w]+'_F814WandF475W_'+model+'_m1'+str(q)+'_m2'+str(j)+'_output.fits\n')
...
if model == 'sersic' and togetherness == 'simultaneous':
    ...
    for w in range(0,ngal):
        for j in range(0,nmag):
            for q in range(0,nmag):
                text.write('galfitm '+galaxies[w]+'_F814WandF475W_'+model+'_m1'+str(q)+'_m2'+str(j)+'_input.txt\n')
    text.close()
