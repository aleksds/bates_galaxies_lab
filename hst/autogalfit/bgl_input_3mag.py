...
re_vals = np.zeros((3, len(bgl_values)))
re_vals[0] = bgl_values['re00']
re_vals[1] = bgl_values['re03']
re_vals[2] = bgl_values['re05']
    ...
    for w in range(0,ngal):
        for i in range(0,1):
            ...
            for q in range(0, nre):
                ...
                file = time+'_'+model+'_'+plate+'_'+togetherness+'_'+imgsize+'_psf'+psf+'/'+galaxies[w]+'_F814W_F475W_F160W_'+model+'_re'+str(q)+'_input.txt'

                text = open(file,'w')
                
                text.write('#  Input menu file: '+galaxies[w]+'_F814W_F475W_F160W_'+model+'_re'+str(q)+'_input.txt\n') #propably not essential
                text.write('#  Chi^2/nu = ,  Chi^2 = ,  Ndof = \n') #probably not essential
                text.write('# IMAGE and GALFIT CONTROL PARAMETERS\n') #will not change , probably not essential
                ...
                text.write('B) '+galaxies[w]+'_F814W_F475W_F160W_'+model+'_re'+str(q)+'_output.fits\n')
                ...
                if model == 'sersic':
                    ...
                    text.write(' 3) '+str(bgl_values[w][2])+','+str(bgl_values[w][1])+','+str(bgl_values[w][3])+'     1,1,1                 band\n')
...
if model == 'sersic' and togetherness == 'simultaneous':
    ...
    for w in range(0,ngal):
        for q in range (0,nre):
            text.write('galfitm '+galaxies[w]+'_F814W_F475W_F160W_'+model+\
            '_re'+str(q)+'_input.txt\n')
    text.close()
