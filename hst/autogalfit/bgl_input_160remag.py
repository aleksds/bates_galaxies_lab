    ...
    for w in range(0,ngal):
        ...
        for j in range(0,nmag):
            for q in range(0, nre):
                ...
                file = time+'_'+model+'_'+plate+'_'+togetherness+'_'+imgsize+'_psf'+psf+'/'+galaxies[w]+'_F814W_F475W_F160W_'+model+'_re'+str(q)+'_mag'+str(j)+'_input.txt'

                text = open(file,'w')
                
                text.write('#  Input menu file: '+galaxies[w]+'_F814W_F475W_F160W_'+model+'_re'+str(q)+'_mag'+str(j)+'_input.txt\n') #propably not essential
                text.write('#  Chi^2/nu = ,  Chi^2 = ,  Ndof = \n') #probably not essential
                text.write('# IMAGE and GALFIT CONTROL PARAMETERS\n') #will not change , probably not essential
                
                text.write('A) '+hstdir+\
                longgal[w]+'/'+plate+'/'+filters[1]+'/final_'+filters[1]+\
                '_drc_sci.fits,'+hstdir+longgal[w]+\
                ...
                text.write('B) '+galaxies[w]+'_F814W_F475W_F160W_'+model+'_re'+str(q)+'_mag'+str(j)+'_output.fits\n')
                #text.write('C) none,none      0.000\n')
                text.write('C) '+sigma_file_814[0]+','+sigma_file_475[0]+','+sigma_file_160[0]+' \n') 
                text.write('D) '+hstdir+longgal[w]+\
                '/'+psf+'/'+filters[1]+'/final_psf.fits,'+hstdir+\
                longgal[w]+'/'+psf+'/'+filters[0]+'/final_psf.fits,'+hstdir+longgal[w]+'/'+psf+'/'\
                ...
                if model == 'sersic':
                    ...
                    contraint_text.write('1              MAG_J     \
                            '+str(md3f[w][j])+' to '+str(md3f[w][j])+' \n')                    
                    contraint_text.write('1              q           '+\
                    str(bgl_values['ba'][w])+' to '+str(bgl_values['ba'][w])+'  \n')
                    contraint_text.write('1              pa          '+\
                    str(bgl_values['pa'][w])+' to '+str(bgl_values['pa'][w])+'  \n')
                ...
                if model == 'sersic':
                    ...
                    text.write(' 3) '+str(bgl_values['m814'][w])+','+str(bgl_values['m475'][w])+','+str(md3f[w][j])+'     1,1,1                 band\n')
...
if model == 'sersic' and togetherness == 'simultaneous':
    ...
    for w in range(0,ngal):
        for j in range(0,nmag):
            for q in range (0,nre):
                text.write('galfitm '+galaxies[w]+'_F814W_F475W_F160W_'+model+'_re'+str(q)+'_mag'+str(j)+'_input.txt\n')
    text.close()
