...
# this version is for when we want to explore parameter space for two magnitudes

# run bgl_input_magir.py sersic coarse 100 coarse simultaneous s_index
...
# md1f stands for "magnitude difference of 1st filter" (i.e., F160W) 
...
for i in range(0, ngal):
    for j in range(0, nmag):
        md1f[i,j] = bgl_values['m160'][i] + mag_array[j]
...
#SIMULTANEOUS FITS OF VARYING DEGREE
    ...
    for w in range(0,ngal):
        ...
        sigma_file_160 = glob.glob(hstdir+'sigma/'+longgal[w]+'_'+filters[0]+'_'+plate+'_sigma.fits')
        if not os.path.exists(time+'_'+model+'_'+plate+'_'+togetherness+'_'+imgsize+'_psf'+psf):
            os.makedirs(time+'_'+model+'_'+plate+'_'+togetherness+'_'+imgsize+'_psf'+psf)

        for j in range(0, nmag):
            #for q in range(0,nre):
            for q in range(0, nmag):
                
                file = time+'_'+model+'_'+plate+'_'+togetherness+'_'+imgsize+'_psf'+psf+'/'+galaxies[w]+'_F814WandF160W_'+model+'_m1'+str(q)+'_m2'+str(j)+'_input.txt'
                text = open(file,'w')
    
                text.write('#  Input menu file: '+galaxies[w]+'_F814WandF160W_'+model+'_m1'+str(q)+'_m2'+str(j)+'_input.txt\n')
                text.write('A) '+hstdir+longgal[w]+'/'+plate+'/'+filters[1]+'/final_'+filters[1]+'_drc_sci.fits,'+hstdir+longgal[w]+'/'+plate+'/'+filters[0]+'/final_'+filters[0]+'_drz_sci.fits\n')
                text.write('A1) V,J\n')
                text.write('A2) 814.000,1600.000\n')
                text.write('B) '+galaxies[w]+'_F814WandF160W_'+model+'_m1'+str(q)+'_m2'+str(j)+'_output.fits\n')
                text.write('C) '+sigma_file_814[0]+','+sigma_file_160[0]+' \n') 
                ...
                if model == 'sersic':
                    #text.write('G) \n')
                    constraintfile= time+'_'+model+'_'+plate+'_'+togetherness+'_'\
                    +imgsize+'_psf'+psf+'/'+'parameters_constraints_'+galaxies[w]+'_m1'+str(q)+'_m2'+str(j)+'.txt'
                
                    constraint_text = open(constraintfile, 'w')
                    constraint_text.write('# Component/    parameter   constraint\
                    Comment\n')
                    constraint_text.write('# operation	(see below)   range\n\n')
                    constraint_text.write('1              n              4 to 4\
                    # Soft constraint: Constrains the\n')
                    constraint_text.write('					# sersic index n to\
                    within values\n')
                    constraint_text.write('				        # from 0.7 to\
                    5.\n\n')
                    #constraint_text.write('1              re         '\
                    #    +str(tmp[w][q])+' to '+str(tmp[w][q])+' \n')
                    constraint_text.write('1              re         '\
                        +str(bgl_values['re'][w]/2.)+' to '+str(bgl_values['re'][w]/2.)+' \n')                    
                    constraint_text.write('1              MAG_V     \
                            '+str(md2f[w][j])+' to '+str(md2f[w][j])+' \n')
                    #constraint_text.write('1              MAG_U     \
                    #        '+str(bgl_values['m475'][w])+' to '+str(bgl_values['m475'][w])+' \n')
                    constraint_text.write('1              MAG_J     \
                            '+str(md1f[w][q])+' to '+str(md1f[w][q])+' \n') 
                    constraint_text.write('1              q           '+\
                    str(bgl_values['ba'][w])+' to '+str(bgl_values['ba'][w])+'  \n')
                    constraint_text.write('1              pa          '+\
                    str(bgl_values['pa'][w])+' to '+str(bgl_values['pa'][w])+'  \n')
                    constraint_text.close()
                
                    text.write('G) parameters_constraints_'+galaxies[w]+'_m1'+str(q)+'_m2'+str(j)+'.txt\n')
                ...
                if model == 'sersic':
                    ...
                    if togetherness == 'semi':
                        text.write(' 1) '+xcoor+','+xcoor+'    1,0                 band\n')
                        text.write(' 2) '+ycoor+','+ycoor+'    1,0                 band\n')
                    text.write(' 3) '+str(md2f[w][j])+','+str(md1f[w][q])+'     1,1                 band\n')
                    text.write(' 4)  '+str(bgl_values['re'][w]/2.)+',0    1,0                 cheb\n')
...
if model == 'psf' and togetherness == 'simultaneous':
    ...
    text.write('source ~/.bash_profile\n')
    for w in range(0,ngal):
        text.write('galfitm '+galaxies[w]+'_F814WandF160W_'+model+'_input.txt\n')
    text.close()
...
if model == 'sersic' and togetherness == 'simultaneous':
    ...
    for w in range(0,ngal):
        for j in range(0,nmag):
            for q in range(0,nmag):
                text.write('galfitm '+galaxies[w]+'_F814WandF160W_'+model+'_m1'+str(q)+'_m2'+str(j)+'_input.txt\n')
    text.close()
