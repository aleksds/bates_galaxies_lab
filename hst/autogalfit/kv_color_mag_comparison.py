...
with PdfPages(name_cc) as pdf:   
    ...
    plt.ylabel('mag_F475W - mag_F814W (Color)')
    plt.title('Color vs Chi Square/Nu Plot')
    plt.legend(loc='upper right')
    pdf.savefig()
    plt.close()
...
######
    ...
    plt.ylabel('color_'+one+'-color_'+two)
    ...
    plt.ylabel('Ratio of Chi Square/Nu Values')
    plt.title('Chi Square/Nu Comparison')
    plt.legend(loc='lower right')
    plt.xlim(x6[0],x6[1])
    plt.ylim(y6[0],y6[1])
    pdf.savefig()
