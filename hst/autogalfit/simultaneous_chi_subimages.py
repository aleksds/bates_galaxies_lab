...
for i in range(0, len(fits_files)):
    ...
    with open(band_files[i]) as f:
        ...
        chi_from_our_calculations[i] = (np.sum((v_res_image_stamp/v_unc_image_stamp)**2)+np.sum((v_res_image_stamp/v_unc_image_stamp)**2))/(((len(v_res_image_stamp)+len(u_res_image_stamp))**2)*1)
        #print(content[47])
