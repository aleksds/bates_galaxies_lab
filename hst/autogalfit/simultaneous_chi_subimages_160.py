...
for i in range(0, len(fits_files)):
    ...
    with open(band_files[i]) as f:
        ...
        v_unc, v_unc_head = hdu[12].data, hdu[12].header
        v_res, v_res_head = hdu[6].data, hdu[6].header

        v_unc_image_stamp = v_unc[(ycen-dy):(ycen+dy), (xcen-dx):(xcen+dx)]
        v_res_image_stamp = v_res[(ycen-dy):(ycen+dy), (xcen-dx):(xcen+dx)]

        u_unc, u_unc_head = hdu[13].data, hdu[13].header
        u_res, u_res_head = hdu[7].data, hdu[7].header

        u_unc_image_stamp = u_unc[(ycen-dy):(ycen+dy), (xcen-dx):(xcen+dx)]
        u_res_image_stamp = u_res[(ycen-dy):(ycen+dy), (xcen-dx):(xcen+dx)]

        j_unc, j_unc_head = hdu[14].data, hdu[14].header
        j_res, j_res_head = hdu[8].data, hdu[8].header
        ...
        chi_from_our_calculations[i] = (np.sum((u_res_image_stamp/u_unc_image_stamp)**2)+np.sum((v_res_image_stamp/v_unc_image_stamp)**2)+np.sum((j_res_image_stamp/j_unc_image_stamp)**2))/(len(v_res_image_stamp)**2 * 3)

        chi_one_our_calculations[i] = np.sum((j_res_image_stamp/j_unc_image_stamp)**2)/(((len(j_res_image_stamp))**2)*1)
        
        #print(content[47])
        #mag[i] = np.float(content[47][4:13])
        mag[i] = np.float(content[47][18:24])
...
name = 'chi_values_'+gal+'_'+band+'_160.pdf'
