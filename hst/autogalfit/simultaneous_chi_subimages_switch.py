...
for i in range(0, len(fits_files)):
    ...
    with open(band_files[i]) as f:
        ...
        chi_one_our_calculations[i] = np.sum((u_res_image_stamp/u_unc_image_stamp)**2)/(((len(u_res_image_stamp))**2)*1)
        
        #print(content[47])
        #mag[i] = np.float(content[47][4:13])
        mag[i] = np.float(content[47][11:17])
...
name = 'chi_values_'+gal+'_'+band+'_switch.pdf'
