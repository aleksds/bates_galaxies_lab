...
def chi_phot(phot_mu, obs, **extras):
    ...
    mask = obs.get('cmask', slice(None))
    #delta = (np.array(obs['mcur']) - np.array(phot_mu))[mask]
    delta = np.ma.masked_where(not obs['cmask'], np.array(obs['mcur']) - np.array(phot_mu))
    #unc = obs['ecur'][mask]
    unc = np.ma.masked_where(not obs['cmask'], np.array(obs['ecur']))
    chi = delta / unc
    return chi
