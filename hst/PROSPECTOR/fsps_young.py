...
#sp_lowz = fsps.StellarPopulation(compute_vega_mags=False, zcontinuous=1,
#                                sfh=0, logzsol=-0.3, dust_type=2, dust2=0.0)
#sp_higz =  fsps.StellarPopulation(compute_vega_mags=False, zcontinuous=1,
#                                sfh=0, logzsol=0.3, dust_type=2, dust2=0.0)
...
with PdfPages(filename) as pdf:
    ...
    t = np.array([0.004,0.006,0.008,0.01,0.02,0.03,0.04,0.06,0.08,0.1,0.2,0.3,0.4])
    ...
    for j in range(0,len(catalog)):
        ...
        #mags_lowz = np.zeros([len(t),len(bands)])
        #mags_higz = np.zeros([len(t),len(bands)])
        for i in range(0,len(mags)):
            ...
            #mags_lowz[i] = sp_lowz.get_mags(tage=t[i], bands=bands, redshift=catalog['z'][j])
            #mags_higz[i] = sp_higz.get_mags(tage=t[i], bands=bands, redshift=catalog['z'][j])
        fig = plt.figure()
        ...
        print(mass_dust)
        print(mass_must)
        #mass_lowz = 10**((m814 - mags_lowz[:,1])/(-2.5))
        #mass_higz = 10**((m814 - mags_higz[:,1])/(-2.5))
        ...
        #plt.scatter(mags_lowz[:,1]-mags_lowz[:,2], mags_lowz[:,0]-mags_lowz[:,1], color='orange', marker='p', label='logz=-0.3')
        #plt.scatter(mags_higz[:,1]-mags_higz[:,2], mags_higz[:,0]-mags_higz[:,1], color='violet', marker='h', label='logz=0.3')
        ...
        #plt.scatter(mass_lowz,t*1e3,color='orange', marker='p', label='logz=-0.3')
        #plt.scatter(mass_higz,t*1e3,color='violet', marker='h', label='logz=0.3')
