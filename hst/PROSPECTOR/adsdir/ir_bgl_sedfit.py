...
run ir_bgl_sedfit.py --nproc=4 --priors='delayed-tau' --galaxy='J0901' --sedfit  --verbose
run ir_bgl_sedfit.py --nproc=4 --priors='delayed-tau' --galaxy='J0901' --qaplots --verbose
...
def load_obs(seed=1, nproc=1, nmin=10, verbose=False, sps=None, galaxy=None):
    ...
    phot = dict(
        FUV=(flux(table.field('fuv_mag')[match][0] - table.field('ebv')[match][0] * 6.892),
             ivar(table.field('fuv_mag')[match][0] - table.field('ebv')[match][0] * 6.892, table.field('fuv_unc')[match][0]),
             0.1528),
        NUV=(flux(table.field('nuv_mag')[match][0] - table.field('ebv')[match][0] * 6.738),
             ivar(table.field('nuv_mag')[match][0] - table.field('ebv')[match][0] * 6.738, table.field('nuv_unc')[match][0]),
             0.2271),
        u=(flux(table.field('u_mag')[match][0] - table.field('ebv')[match][0] * 4.278),
           ivar(table.field('u_mag')[match][0] - table.field('ebv')[match][0] * 4.278, table.field('u_unc')[match][0]),
           0.3543),
        g=(flux(table.field('g_mag')[match][0] - table.field('ebv')[match][0] * 3.294),
           ivar(table.field('g_mag')[match][0] - table.field('ebv')[match][0] * 3.294, table.field('u_unc')[match][0]),
           0.4770),
        r=(flux(table.field('r_mag')[match][0] - table.field('ebv')[match][0] * 2.393),
           ivar(table.field('r_mag')[match][0] - table.field('ebv')[match][0] * 2.393, table.field('u_unc')[match][0]),
           0.6231),
        i=(flux(table.field('i_mag')[match][0] - table.field('ebv')[match][0] * 1.859),
           ivar(table.field('i_mag')[match][0] - table.field('ebv')[match][0] * 1.859, table.field('u_unc')[match][0]),
           0.7625),
        z=(flux(table.field('z_mag')[match][0] - table.field('ebv')[match][0] * 1.346),
           ivar(table.field('z_mag')[match][0] - table.field('ebv')[match][0] * 1.346, table.field('u_unc')[match][0]),
           0.9134),
        w1=(flux(table.field('w1_mag')[match][0] - table.field('ebv')[match][0] * 0.171) * 306.681 / 3631,
            ivar(table.field('w1_mag')[match][0] - table.field('ebv')[match][0] * 0.171, table.field('w1_unc')[match][0]) * (3631 / 306.681) ** 2,
            3.368),
        w2=(flux(table.field('w2_mag')[match][0] - table.field('ebv')[match][0] * 0.096) * 170.663 / 3631,
            ivar(table.field('w2_mag')[match][0] - table.field('ebv')[match][0] * 0.096, table.field('w2_unc')[match][0]) * (3631 / 170.663) ** 2,
            4.618),
...
def load_model(obs, template_library='delayed-tau', verbose=False):
    ...
    model_params['add_duste'] = {"N": 1, "isfree": False, "init": True}
    ...
    model_params['duste_gamma'] = {"N": 1, "isfree": True,
...
def main():
    ...
    hfile = os.path.join(j2118dir, 'ir-{}-{}.h5'.format(args.galaxy, args.priors))
    ...
    if args.qaplots:
        ...
        png = os.path.join(j2118dir, 'ir-{}-{}-sed.png'.format(args.galaxy, args.priors))
        bestfit_sed(obs, chain=result['chain'], lnprobability=result['lnprobability'], 
                    sps=sps, model=model, seed=1, nrand=100, png=png)
    
        pdb.set_trace()
        
        png = os.path.join(j2118dir, 'ir-{}-{}-corner.png'.format(args.galaxy, args.priors))

        #subtriangle(result, showpars=['logmass', 'tage', 'tau', 'dust2', 'dust_ratio'],
        #            logify=['tau'], png=png)
        
        if args.priors=='delayed-tau':
            subtriangle(result, showpars=['logmass', 'tage', 'tau', 'dust2', 'dust_ratio', 'duste_umin', 'duste_qpah', 'duste_gamma'], png=png)
