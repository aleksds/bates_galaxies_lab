...
def bestfit_sed(obs, chain=None, lnprobability=None, theta=None, sps=None,
    ...
    *or* theta (to visualize just a single SED fit).
    ...
    #spec, phot, extras = sps.get_spectrum(outwave=modelwave / (1+obs['redshift']), #obs['wavelength'],
    #                                          filters=tobs['filters'])#,
    ...
    ax.errorbar(wteff, mags, marker='s', ls='', lw=3, markersize=15, markerfacecolor='none',
...
parser.add_argument('--galaxy', default='J0826', type=str, help='Galaxy name.')
