...
def find_g(plate_rec, min_p, max_p):
    ...
    for i in range(0, len(lines)):
        ...
        for j in range(0, 1):
            ...
            # bradna_index = np.abs(
            #     (((vertical_distance_map * ((inc * 180 / np.pi) ** 6)) ** 4) * ((10 * oii_flux) ** 2)) * ((vertical_distance_map ** 2) * oii_to_ha) *
            ...
            bradna_index = np.abs(((10 * oii_flux) ** 2) * (oii_to_ha ** 2) * (oii_flux / dap_serr) * (vertical_distance_map ** .1))  # * (normalize_vdist ** 2))
            ...
            print(plate, galaxy, 'B_index:', b_index, state, 'Rating: ',
                  b_index * (100 / 6))  # math.log(bradna_index_sum, 10)
            ...
            if state == '>>>>>>>>>>>>>>>>Interesting':
                ...
                int_gal = [plate, galaxy, (b_index * (100 / 6)), gal_indx]
                print(int_gal)
...
for each_plate in range(start_p, fin_p):
    interesting = find_g(all_plates[each_plate], min_p, max_p)
