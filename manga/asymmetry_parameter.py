...
poop = np.zeros(total)
poo_p = np.zeros(total)
poo_n = np.zeros(total)

filename = 'xi_plotg.pdf'
    ...
    for i in range(0,total):
        ...
        if os.path.isfile(name):
            ...
            for j in range(0, 1):
                ...
                frac = delta_vel/denom                                                  # asymmetry parameter before taking the std
                ...
                plt.text(0, -8, r'$η_{50} =$' + ' ' + str(med_vel_disp_vrot_max), fontsize=8)
                pdf.savefig()
                plt.close()
