...
def main(plate_rec):
    ...
    with PdfPages(filenamez) as pdf:
        for i in range(0, len(lines)):
            ...
            for j in range(0, 1):
                ...
                # Galaxy rating
                bradna_index = np.abs(zproj_kpc_map*dap_sflux/dap_ha_sflux*dap_sflux/dap_serr*inc*180/np.pi/1e7)
                param = (bradna_index>0)
                bradna_index = np.sum(bradna_index[param])
                if (inc*180/np.pi)>65:
                    state = '>>>>>>>>>>>>>>>>Interesting'
                else:
                    state = 'Not Interesting'
                print(plate,galaxy,'Rating:',bradna_index,state,'\n')
...
for each_plate in range(start_p, fin_p):
    ret_pdf = main(all_plates[each_plate])
    merger.append(ret_pdf)
