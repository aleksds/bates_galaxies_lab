...
good_plates = np.sort(plateifu[blah][late][edge])   # all the manga ifu plates
    ...
    for i in range(total):
        ...
        if os.path.isfile(name):
            ...
            for j in range(0, 1):
                ...
                plt.title('(' + str(good_plates[i]) + ')' + '    ' + r'$v_{gas}$', verticalalignment='center', fontsize=5)
                ...
                plt.title(r'$v - v_{flip}$' + '  ' + str(round(disp[i], 1)), verticalalignment='center', fontsize=5)
                ...
                plt.imshow(delta_vel, origin='lower',
                           interpolation='nearest',
                           cmap=cm.coolwarm, vmin=-40, vmax=40)
                plt.text(0, -5, r'$ξ =$' + ' ' + str(round(assym)), fontsize=8)
                plt.title((r'$v - v_{flip}$' + ' arcsec = 2 '), verticalalignment='center', fontsize=5)
                divider = make_axes_locatable(ax)
                cb = plt.colorbar(fraction=0.045, pad=0.02)
                cb.ax.set_yticklabels(cb.ax.get_yticklabels(), fontsize=3)
                ...
                plt.title(r'$v_{disp}$ ' + ' ' + str(round(med_vel_disp_vrot_max,1)), fontsize=5)
                cb = plt.colorbar(fraction=0.045, pad=0.02)
                cb.ax.set_yticklabels(cb.ax.get_yticklabels(), fontsize=3)
