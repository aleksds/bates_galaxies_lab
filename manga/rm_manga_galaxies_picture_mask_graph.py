...
# creates variables that provide directory for information
...
# creates array of just b/a ratios of all galaxies in drpall file
ba = drpdata.field('nsa_sersic_ba')
# creates array of just the plate-ifu numbers of all galaxies in drpall file
...
# creates boolian array where true location corresponds to the galaxies with wanted disk concentration
th50_manga = th50[idx]
th90_manga = th90[idx]
...
# an array of all the MaNGA late type edge on galaxies
good_plates = np.sort(plateifu[blah][late][edge])




filename = 'galaxies_all_spaxel_mask.pdf'
with PdfPages(filename) as pdf:
    for i in range(0,len(good_plates)):  # len(good_plates)):
        ...
        if os.path.isfile(name):
            ...
            for j in range(0, 1):
                ...
                # plot 3: emission-line SFLUX signal-to-noise ratio
                ax = fig.add_subplot(3, 3, 3)
                daplot(dap_sflux / dap_serr, 0.1, 10.)
                plt.title('s/n ratio of O[II] flux', fontsize=8)



                # plot 4: emission-line velocity
                ...
                plt.colorbar()
                plt.title('GVEL-VFLIP', fontsize=8)



                # plot 7: emission-line SFLUX signal-to-noise ratio
                ax = fig.add_subplot(3, 3, 7)
                daplot(s_n, 0.1, 10.)
                plt.title('s/n ratio of O[II] flux w/ spaxel mask', fontsize=8)
                ...
                for m in range(0, size):
                    for n in range(0, size):
                        dist = np.sqrt(
                            (xproj_kpc - xproj_kpc[m, n]) ** 2 + (yproj_kpc + yproj_kpc[m, n]) ** 2)
                        test = (dist == np.min(dist))
                        vel_flip[m, n] = dap_vel[test]
                ...
                plt.imshow(dap_vel, origin='lower',
                           ...
                           cmap=cm.coolwarm, vmin=-250, vmax=250)
                plt.colorbar()
                plt.title('GVEL w/ spaxel mask', fontsize=8)
