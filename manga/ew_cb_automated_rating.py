...
def plot_g(galaxy_c):
    ...
    with PdfPages(current_p) as pdf:
        ...
        for j in range(0, 1):
            ...
            #Printing information for analysis. This bit should be temporary.
            #
            # print(int_gal, ' Inclination: ', inc * 180 / np.pi)
            # non_zero = oii_flux > 0
            # print("OIId flux sum", np.sum(oii_flux))
            # print("Length of flux elements", len(oii_flux[non_zero]), " Mean value of flux elements ", np.mean(oii_flux))
            # print("Sum of oii-to-ha level 1 strain ", np.sum(oii_to_ha), ' Mean: ', np.mean(oii_to_ha))
            # print("Sum of vertical max", np.amax(np.abs(zproj_kpc_map)))
            # print("Sum of s/n", np.sum(oii_flux / dap_serr), ' Mean: ', np.mean(oii_flux / dap_serr))
            # print("Sum of ha", np.sum(dap_ha_sflux), ' Mean: ', np.mean(dap_ha_sflux))
            # print("Index sum: ", bradna_index_sum, ' bradna_index mean: ', np.mean(bradna_index))
            # print("Flux weight mean: ", flux_w_mean, ' Ratio weight mean :', ratio_w_mean)
            # print("Oii Quality: ", oii_quality, ' Oii Q Lvl 2: ',oii_quality_2," Oii_ha Q level 1: ", oii_ha_quality_1, " Oii_ha Q level 2",
            #       oii_ha_quality_2, " Global quality: ", global_quality, '\n')
            ...
            # ax.set_xticklabels(())
            # ax.set_yticklabels(())
            # daplot((((10 * oii_flux) ** 2) * (oii_to_ha ** 3.5) * (oii_flux / dap_serr) * disk_filter),0.1, 30000)
            #
            # # plt.imshow(dap_sig, origin='lower',
            # #            interpolation='nearest',
            #            cmap=cm.YlOrRd, vmin=0, vmax=250)
            # plt.colorbar()
            plt.title('Oii_ha_2', fontsize=8)
...
with open('sorted_rated.pdf', 'wb') as fout:
    merger.write(fout)
...
os.system("open %s &" % 'sorted_rated.pdf')
