...
gal_info = ascii.read(dir2+'king_info.txt')
gal = gal_info['gal']
vcen = gal_info['vcen']
z = gal_info['zem']
...
with PdfPages('boop') as pdf:
    for j in range(0, len(gal)):
        ...
        # HIRES spectrum 
        plt.plot((wave2[0]/(1+z[j])), flux2, color = 'orange')
        plt.plot((wavelength[0]/(1+z[j])), flux_cor, color = 'blue')
