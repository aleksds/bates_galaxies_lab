...
#COLUMN Limits        
        ...
        lim_clmn = [col_2796, col_2802, col_2852]
#Error Limits        
...
##Flux Plot
        ...
        # #Actual Flux vs. Velocity Plot
        # ax.plot(vel_kms[2][g2852], flux[g2852], linewidth=1, label = names[2], color = '#947e94')
        # ax.set_xlim(-3000, 500)
        # ax.set_ylim(0, 2)
        # # plt.legend(loc = 3)
...
def FLUX(velocityy, fluxx, namess, hue):
    ...
    for i in len(namess):
        ...
        ax = fig.add_subplot(2,1,1)
        ax.plot(velocityy[i][limit], fluxx[limit], linewidth=1, label = namess[i], color = hue[i])
        ax.set_xlim(-3000, 500)
        ax.set_ylim(0, 2)
        plt.legend(loc = 3)
        plt.title("Flux Plot for Galaxy %s" %(gal[h]))
...
##Tau Plot
        ...
        #Actual Flux vs. Velocity Plot
        ax.plot(vel_kms[2][g2852], tau[g2852], linewidth=1, label = names[2], color = '#947e94')
        ax.set_xlim(-3000, 500)
        # y-axis upper lim set to 5 because no visible difference between tau = 5 and tau = infinity
        ax.set_ylim(-.2, 5)
